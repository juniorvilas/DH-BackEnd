...
import com.digitalhouse.clinicaodonto.dao.configuracao.ConfigurationJDBC;
...
public class DentistaServiceTest {
    ...
    private static DentistaService dentistaService = new DentistaService(
            new DentistaDaoH2(new ConfigurationJDBC()));

    @BeforeClass
    public static void carregarDados() {
        dentistaService.cadastrar(new Dentista(
                "Sebastião", "Luz", 3455647));
    }

    @Test
    public void cadastrarTest() {
        Dentista dentista = dentistaService.cadastrar(new Dentista(
                "João", "Pereira", 348971960));
